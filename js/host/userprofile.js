  ...
  $.ajax({
    ...
    $.getJSON('http://localhost:8080/getUser/' + id, function (res) {
      $.each(res, function (index) {
        ...
        $('#profileImage').append('<img src="http://localhost:8080/uploads/' + res[index].image_name +
          '" alt="IMG" style=" width: 250px;  border-radius: 125px; "  >');
        $('#button').append(
          '<a href="updateProfile.html"><button class="submitButton" type="submit" name="Submit" id="">Edit Profile</button></a>'
          );
      });
    });
